...
class Pokedex():
    ...
    def load_save_file(self, filename):
        ...
        filename = input("Enter the save file name to load (default: pokemon.json): ")
        if not filename:
                filename = "pokemon.json"
        file_type = filename.split('.')
        if len(file_type) < 2:
            print("Invalid file name. Please include a file extension (e.g., .txt or .json).")
            trainer1.pokedex.load_save_file("pokemon.json")
            return
        if file_type[1] == "json" or file_type[1] == "txt":
            try:
            ...
            try:
                with open(filename, 'r') as file:
                    ...
                    for item in data:
                        ...
                        National_number = item.get("National_number", 0)
                        Type = item.get("Type", "Unknown")
                        species = item.get("species", "Unknown")
                        Height = item.get("Height", "0.0m")
                        Weight = item.get("Weight", "0.0kg")
                        Abilities = item.get("Abilities", [])
                        total = item.get("total", 0)
                        hp = item.get("hp", 0)
                        attack = item.get("attack", 0)
                        Defense = item.get("Defense", 0)
                        sp_attack = item.get("sp_attack", 0)
                        sp_defense = item.get("sp_defense", 0)
                        speed = item.get("speed", 0)

                        pokemon = Pokemon(name, National_number, Type, species, Height, Weight, Abilities, total, hp, attack, Defense, sp_attack, sp_defense, speed)
                        loaded_pokemon.append(pokemon.name)
                        self.PokemonList.append(pokemon)
                    print(loaded_pokemon)
                    print("save file loaded successfully.")
                ...
                print("Save File not found.")
        if not filename:
            print("No filename provided. Skipping load.")

    def graph_stats(self):
        # Collect all relevant stats for each Pokémon
        stats = {
            "hp": [],
            "attack": [],
            "Defense": [],
        ...
        for pokemon in self.PokemonList:
            stats["hp"].append(pokemon.hp)
            stats["attack"].append(pokemon.attack)
            stats["Defense"].append(pokemon.Defense)
            stats["sp_attack"].append(pokemon.sp_attack)
            stats["sp_defense"].append(pokemon.sp_defense)
            stats["speed"].append(pokemon.speed)
            stats["name"].append(pokemon.name)
            stats["Type"].append(pokemon.Type)
            stats["total"].append(pokemon.total)
        print(stats)

        #pie chart for pokemon types
        #count each type occurence
        type_counts = {}
        for t in stats["Type"]:
            type_counts[t] = type_counts.get(t, 0) + 1

        plt.pie(type_counts.values(), labels=type_counts.keys(), autopct='%1.1f%%')
        plt.title("Pokémon Types Distribution")
        ...
        #bar chart for pokemon total stats
        plt.bar(stats["name"], stats["total"])
        plt.title("Pokémon Total Stats")
        plt.xlabel("Pokémon")
        ...
        for stat in stat_names:
            plt.plot(stats["name"], stats[stat])
            plt.title("Pokémon {0} Stats".format(stat.capitalize()))
            plt.xlabel("Pokémon")
            plt.ylabel("{0} Stats".format(stat.capitalize()))
            plt.xticks(rotation=90)
            plt.tight_layout()

            # Save the bar chart as a PNG file
            plt.savefig("pokemon_{0}_stats.png".format(stat))
            plt.show()
            # Clear the current figure
            plt.clf()
...
class Pokemon():
    ...
    def Edit_Pokemon(self):
        ...
        int_attributes = ["hp", "attack", "Defense", "sp_attack", "sp_defense", "speed"]
        ...
        if user_intput in attributes_available:
            if user_intput == "Abilities":
            ...
            elif user_intput == "National_number":
                new_value = input("Please enter the new value for {0} here (4 didgit integer only): ".format(user_intput))
                try:
                    new_value = int(new_value)
                    
                except ValueError:
                    print("Invalid value for {0}, no change made.".format(user_intput))
                    print("Exiting edit mode.")
                    return self
                new_value = str(new_value).zfill(4)  # Pad with leading zeros to ensure 4 digits
                print(new_value)



            elif user_intput == "Weight":
                new_value = input("Please enter the new value for {0} here (include units, e.g., '56.4kg'): ".format(user_intput))
                if new_value.endswith("kg"):
                    new_value = new_value[:-2]  # Remove the 'kg' for conversion
                    try:
                ...
                try:
                    ...
                    print("Exiting edit mode.")
                    return self
                new_value = str(new_value) + "kg"

                #height same as weight but different units
            elif user_intput == "Height":
                new_value = input("Please enter the new value for {0} here (include units, e.g., '10.0m'): ".format(user_intput))
                if new_value.endswith("m"):
                    new_value = new_value[:-1]  # Remove the 'm' for conversion
                    try:
                        new_value = float(new_value)
                    except ValueError:
                        print("Invalid value for {0}, no change made.".format(user_intput))
...
def StartMenu(trainer1):
    while True:
        ...
        time.sleep(0.02)
        print("Option 5: Generate graphs for pokemon stats")
        time.sleep(0.02)
        ...
        #system exit option
            ...
            if user_input.lower() == "y":
                #make save file as both json and txt
                filename = "pokemon.json"
